...
const CustomerReviews = ({ productId }: Props) => {
    ...
    if (loading) {
        return (
            <div className="w-full mt-12 border-t border-gray-200 pt-8 lg:pt-12 flex justify-center">
    ...
    return (
        <div className="w-full mt-12 border-t border-gray-200 pt-8 lg:pt-12">
            <h2 className="text-xl lg:text-2xl font-bold text-gray-900 mb-6 lg:mb-8">Customer reviews</h2>
            <div className="flex flex-col lg:flex-row gap-8 lg:gap-12">
                {/* Left Column */}
                <div className="w-full lg:w-[35%] flex flex-col gap-6">
                    {/* 100% authentic badge */}
                    <div className="bg-[#eaf4eb] border border-[#d2e8d4] rounded-lg p-4">
                    ...
                    <div>
                        <div className="flex items-end gap-3 mb-1">
                            <span className="text-[40px] leading-none font-bold text-gray-900">{stats.averageRating.toFixed(1)}</span>
                                ...
                                <div className="flex text-yellow-400 gap-1 cursor-pointer">
                                    {[1, 2, 3, 4, 5].map((star) => (
                                        <svg
                                            ...
                                            className={`w-6 h-6 ${star <= formRating ? 'text-yellow-400' : 'text-gray-300'}`}
                                            fill="currentColor"
                                            viewBox="0 0 20 20"
                                        >
                            ...
                            {reviews.map((review) => (
                                <div key={review._id} className="border-b border-gray-200 pb-6">
                                    <div className="flex items-center gap-2 mb-2">
                                        ...
                                        <div>
                                            <div className="font-bold text-sm text-gray-900">{review.user?.name || review.user?.email || 'User'}</div>
