...
export default function FeatureBar() {
    const features = [
        {
            ...
            icon: (
                <img
                    src="/doctor_logo.svg"
                    alt="Dr. Corner"
                    className="w-6 h-6 xs:w-7 xs:h-7 sm:w-11 sm:h-11 shrink-0 mb-1 sm:mb-3 object-contain"
                />
            )
        },
        {
            ...
            icon: (
                <svg className="w-6 h-6 xs:w-7 xs:h-7 sm:w-11 sm:h-11 text-[#458500] shrink-0 mb-1 sm:mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    return (
        <div className="w-full bg-white pt-0 sm:pt-2 overflow-hidden">
            <div className="max-w-[1400px] mx-auto px-1.5 sm:px-6">
                <div className="grid grid-cols-4 gap-1 sm:gap-4 lg:gap-8">
                    {features.map((item) => (
                        <div
                            key={item.id}
                            className="flex flex-col items-center justify-start text-center p-0.5 sm:p-2"
                        >
                            {item.icon}
                            <span className="text-[10px] xs:text-[11px] sm:text-base font-bold text-gray-900 leading-tight mb-0.5 sm:mb-1.5">
                                {item.title}
                            </span>
                            <span className="text-[8px] xs:text-[9px] sm:text-sm text-gray-500 leading-[10px] sm:leading-snug max-w-[210px]">
                                {item.subtitle}
                            </span>
                        </div>
                    ))}
