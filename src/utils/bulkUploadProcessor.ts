...
export async function generateBulkUploadPdfReport(auditRecords: BulkAuditRecord[]): Promise<void> {
    ...
    try {
        ...
        auditRecords.forEach((rec, idx) => {
            const changesList = rec.changes.map(ch => `
                <tr style="border-bottom: 1px solid #e2e8f0;">
                    <td style="padding: 7px 10px; font-weight: 700; color: #334155; font-size: 11px;">${ch.fieldName}</td>
                    <td style="padding: 7px 10px; color: #b91c1c; font-size: 11px; background-color: #fef2f2; word-break: break-word;">${ch.oldValue}</td>
                    <td style="padding: 7px 10px; color: #15803d; font-weight: 700; font-size: 11px; background-color: #f0fdf4; word-break: break-word;">${ch.newValue}</td>
                </tr>
            `).join('');

            productsHtml += `
                <div style="margin-bottom: 18px; page-break-inside: avoid; border: 1px solid #cbd5e1; border-radius: 6px; overflow: hidden; background: #ffffff;">
                    <div style="background-color: #f8fafc; padding: 10px 14px; border-bottom: 1px solid #cbd5e1; display: flex; justify-content: space-between; align-items: center;">
                        <div style="flex: 1;">
                            <span style="font-weight: 800; font-size: 13px; color: #0f172a;">${idx + 1}. ${rec.productName}</span>
                            <span style="margin-left: 10px; font-family: monospace; font-size: 11px; background: #e0f2fe; color: #0369a1; padding: 2px 8px; border-radius: 4px; font-weight: 800;">SKU: ${rec.sku}</span>
                        </div>
                        <span style="font-size: 11px; color: #16a34a; font-weight: 700; background: #dcfce7; padding: 2px 8px; border-radius: 4px;">${rec.changes.length} Field(s) Changed</span>
                    </div>
                    <table style="width: 100%; border-collapse: collapse; text-align: left; font-family: sans-serif;">
                        <thead>
                            <tr style="background-color: #f1f5f9; border-bottom: 1px solid #cbd5e1;">
                                <th style="padding: 7px 10px; font-size: 11px; font-weight: 800; color: #475569; width: 30%;">Field Name</th>
                                <th style="padding: 7px 10px; font-size: 11px; font-weight: 800; color: #dc2626; width: 35%;">Old Value</th>
                                <th style="padding: 7px 10px; font-size: 11px; font-weight: 800; color: #16a34a; width: 35%;">New Value</th>
                            </tr>
                        </thead>
                        <tbody>
                            ${changesList}
        ...
        reportContainer.innerHTML = `
            <div style="border-bottom: 3px solid #16a34a; padding-bottom: 12px; margin-bottom: 20px; display: flex; justify-content: space-between; align-items: flex-end;">
                <div>
                    <h1 style="margin: 0; font-size: 22px; font-weight: 800; color: #16a34a; tracking: -0.5px;">Bulk Upload SKU Change Audit Report</h1>
                    <p style="margin: 4px 0 0 0; font-size: 12px; color: #64748b; font-weight: 600;">Detailed record of modified fields for existing products</p>
                </div>
            ...
            <div style="margin-top: 24px; padding-top: 12px; border-top: 1px solid #cbd5e1; text-align: center; font-size: 10px; color: #64748b; font-weight: 600;">
                Generated automatically by E-Commerce Admin Bulk Upload System • ${dateStr} ${timeStr}
            </div>
        `;
