                                            ...
                                            <div className="flex items-center gap-3 sm:gap-4">
                                                <div className="h-10 w-10 sm:h-12 sm:w-12 flex-shrink-0 bg-gray-100 rounded-md overflow-hidden border border-gray-200 flex items-center justify-center">
                                                    {product.images && product.images.length > 0 ? (
                                                        <img className="h-full w-full object-cover" src={`${process.env.NEXT_PUBLIC_API_URL}/upload/file/${product.images[0]}`} alt={product.name} onError={(e) => { if (!e.currentTarget.src.includes('via.placeholder.com')) { e.currentTarget.src = 'https://via.placeholder.com/150'; } }} />
                                                    ) : (
